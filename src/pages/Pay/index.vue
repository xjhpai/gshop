...
export default {
  ...
  data() {
    return {
      tradeNo: this.$route.query.trade,
      payInfo: {},
    };
  },
  mounted() {
  ...
  methods: {
    ...
    async pay() {
      //弹出盒子->生成二维码->检查支付状态->成功：跳转页面，未支付：停留
      try {
        const imgUrl = await QRCode.toDataURL(this.payInfo.orderId);
        ...
        if (!this.timer) {
          this.timer = setInterval(async () => {
            const result = await this.$API.reqPayStatus(this.tradeNo);
            if (result.code === 200) {
              //支付成功
              this.payStatus = 200; //记录状态
              clearInterval(this.timer);
              this.timer = null;
              this.$router.push("/paysuccess");
              this.$msgbox.close();
